...
function getCMatrix(lmax::Int64)
    ...
    for m=1:(lmax+1), l=1:(lmax+1) # !! ATTENTION, size of the array is (lmax+1) in both directions
        CMatrix[l,m] = 0.0
    end

    CMatrix[1,1] = 1.0/sqrt(4.0*pi) # Ylm(pi/2,0) for (l,m)=(0,0) !! HARMONIC INDEX STARTS AT \ell=0

    # First loop to fill in the diagonal elements
    for l=0:(lmax-1) #!! ATTENTION TO THE BOUNDS OF THE LOOP
        CMatrix[(l+1)+1,(l+1)+1] = - sqrt((2.0*l+3.0)/(2.0*l+2.0))*CMatrix[l+1,l+1]
    end
    ...
    for m=0:lmax # We perform the loop at fixed m
        for l=m:2:(lmax-2) # At fixed m, we scan the \ell by step of 2 !! ATTENTION TO THE BOUNDS OF THE LOOP
            CMatrix[(l+2)+1,m+1] = - sqrt((2.0*l+5.0)/(2.0*l+1.0))*sqrt(((l-m+1.0)*(l+m+1.0))/((l-m+2.0)*(l+m+2.0)))*CMatrix[l+1,m+1]
        end
    end
    return CMatrix
end
