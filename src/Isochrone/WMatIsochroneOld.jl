...
"""
function RunWmatIsochrone(params::LinearParameters)
    ...
    dimension = params.basis.dimension
    nradialmax = params.nradial

    # check if we can cover the specified radial orders
    if params.nradialmax > params.nradial
    ...
    # bases prep.
    bases=[deepcopy(params.basis) for k=1:Threads.nthreads()]

    # Legendre integration prep.
    tabuGLquadtmp,tabwGLquad = FiniteHilbertTransform.tabuwGLquad(params.Ku)
    tabuGLquad = reshape(tabuGLquadtmp,params.Ku,1)

    # Resonance vectors
    nbResVec, tabResVec = MakeTabResVec(params.lharmonic,params.n1max,dimension)
    ...
    Threads.@threads for i = 1:nbResVec
        ...
        if VERBOSE > 1
            @time tabWMat,tabaMat,tabeMat = MakeWmatUVIsochrone(n1,n2,tabuGLquad,params.Kv,params.lharmonic,bases[k],params.Ω₀,params.Kw,bc=bc,M=M,G=G)
        else
            tabWMat,tabaMat,tabeMat = MakeWmatUVIsochrone(n1,n2,tabuGLquad,params.Kv,params.lharmonic,bases[k],params.Ω₀,params.Kw,bc=bc,M=M,G=G)
        end
